...
This directory contains examples demonstrating how to use the URL denylist feature in proxy-server.
...
```bash
npm run start -- --url https://api.example.com --port 8080 --denylist "*.json"
```
...
```bash
npm run start -- --url https://example.com --port 8080 --denylist "*/api/*"
```
...
```bash
npm run start -- --url https://example.com --port 8080 --denylist "https://cdn.example.com/*,https://static.example.com/*"
```
...
When caching is implemented (issues #3 and #5), the denylist will be used to:
